/* changed order: indata= at the end */
%macro getMfr(outlib=work, focus=m, indata=,info=basis, fromyear=1997);

    %let info = %sysfunc(substr(&info,1,5));
	%findingmfr(&outlib..mfr&info.all, &focus, &info, fromyear=&fromyear, indata=&indata);
...
%macro findingmfr(outdata, focus, info=basis, fromyear=, indata=);
	%let sqlrc=0;
			...
			proc sql inobs=&sqlmax;
				...
				%else insert into &outdata;
					select
						...
						from
						%if &indata ne %then (select distinct pnr from &indata) c inner join;
						raw.mfr&yr a
